...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($identification == 'voluntario') {
        ...
        if ($password !== $confirmPassword) {
            header("Location: ../templates/index.html?error=" . urlencode('Las contraseñas no coinciden'));
            exit();
        }
        ...
        if ($result->num_rows > 0) {
            ...
            header("Location: ../templates/index.html?error=" . urlencode('El correo o el RUT ya están registrados'));
            exit();
        }
        ...
        if ($stmt->execute()) {
            header("Location: ../templates/index.html?success=" . urlencode('Registro exitoso'));
        } else {
            // Mostrar error si no se pudo insertar
            header("Location: ../templates/index.html?error=" . urlencode('Error al registrar el voluntario: ' . $stmt->error));
        }
        ...
        if ($stmt->execute()) {
            ...
            header("Location: ../templates/index.html?error=" . urlencode('Error al registrar la organización: ' . $stmt->error));
        }
    } else {
        header("Location: ../templates/index.html?error=" . urlencode('Tipo de identificación no válido'));
    }
}
